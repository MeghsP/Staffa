...
export default class EmploymentContractScreen extends Component {
 ...
 componentDidMount(){
  if(this.context.userData && this.context.userData.isTermsAccepted){
   var empData = this.context.userData.isEmploymentAccepted;
   this.setState({isEmploymentAccepted:empData});
        ...
        <View style = {context.utilities.styles.baseStyle1}>
          ...
          />
            <TouchableOpacity style = {{width:context.screenWidth}} onPress={ () => this.onAgreeClick()}>
              <Text style = {[context.utilities.styles.LoginButtonEnableTextStyle, {margin:5,marginBottom:30}]}>{this.state.isDataAvailable ? 'UPDATE' :  "NEXT"}</Text>
            </TouchableOpacity>
